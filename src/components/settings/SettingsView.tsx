...
export function SettingsView() {
  ...
  useEffect(() => {
    platform.appVersion().then(setAppVersion);
    platform.autostart?.isEnabled().then(setAutostartOn).catch(() => {});
  ...
  async function handleToggleAutostart(value: boolean) {
    try {
      if (value) await platform.autostart?.enable();
  ...
  async function handleApplyShortcut() {
    ...
    try {
      await platform.globalShortcut?.register(accelerator);
  ...
  async function handleChooseBackupFolder() {
    const path = await platform.backupFolder?.choose();
    if (!path) return;
  ...
  return (
    <div className="mx-auto h-full max-w-xl overflow-y-auto py-2">
      ...
      {platform.autostart && platform.globalShortcut && (
      ...
      <section className="mb-6 rounded-[2px] border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        ...
        {!platform.updater && <p className="text-xs text-[var(--color-text-muted)]">{t("settings.webUpdatesNote")}</p>}

        {platform.updater && updater.status === "available" && (
            ...
            {updater.selfUpdateSupported === false ? (
              <div className="mt-2">
                ...
                <Button
                  variant="primary"
                  size="sm"
                  onClick={() => platform.openUrl("https://github.com/RickCaleg/chronos/releases/latest")}
                >
                  <Download size={14} />
        ...
        {platform.updater && updater.status !== "ready" && (
          <Button
            variant="secondary"
            size="sm"
      ...
      <section className="mb-6 rounded-[2px] border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        ...
        <p className="mb-4 text-xs text-[var(--color-text-muted)]">{t(platform.kind === "web" ? "settings.dataDescriptionWeb" : "settings.dataDescription")}</p>
